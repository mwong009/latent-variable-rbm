...
class CRBM(object):
    ...
    def propup(self, vis, v_context):
        ...
        pre_sigmoid_activation = T.dot(vis, self.W) + \
                                 T.dot(v_context, self.B) + self.hbias
        return [pre_sigmoid_activation, T.nnet.sigmoid(pre_sigmoid_activation)]

    def sample_h_given_v(self, v0_sample, v_context):
        ''' This function infers state of hidden units given visible units '''
        # compute the activation of the hidden units given a sample of the
        # visibles
        #pre_sigmoid_h1, h1_mean = self.propup(v0_sample)
        pre_sigmoid_h1, h1_mean = self.propup(v0_sample, v_context)
    ...
    def propdown(self, hid, v_context):
        ...
        function)
        '''
        mean_activation = T.dot(hid, self.W.T) + T.dot(v_context, self.A) + \
                          self.vbias
        return mean_activation

    def sample_v_given_h(self, h0_sample, v_context):
        ''' This function infers state of visible units given hidden units '''
        # compute the activation of the visible given the hidden sample
        #pre_sigmoid_v1, v1_mean = self.propdown(h0_sample)
        v1_mean = self.propdown(h0_sample, v_context)
    ...
    def get_cost_updates(self, lr=0.1, persistent=None, k=1):
        ...
        pre_sigmoid_ph, ph_mean, ph_sample = \
                        self.sample_h_given_v(self.input, self.input_context)
...
def train_crbm(learning_rate=1e-3, training_epochs=300,
    ...
    cost, updates = crbm.get_cost_updates(lr=learning_rate, k=10)
