...
V11 = np.dot(age, b_age) + np.dot(foreigner, b_forgn) + np.dot(european, b_eu) + np.dot(payroll_acc, b_payr) + np.dot(particular, b_parti) + asc

V21 = np.dot(age, b_age) + np.dot(loyalty, b_loyal) + np.dot(active, b_activ) + np.dot(derivada, b_deriv) + np.dot(masparti, b_masp) + np.dot(partiplus,b_parti) + np.dot(e_acc, b_eacc) + asc
